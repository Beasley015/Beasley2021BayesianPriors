...
site.rich <- data.frame(Est = rowSums(Zs.mean))

# Get true and observed values for comparison
site.rich$True <- colSums(tru)
site.rich$Obs <- rowSums(apply(obs.data, c(1,3), max))
site.rich$Cov <- cov # Covariate makes for a cleaner plot

# Coerce to long format
rich.long <-pivot_longer(site.rich, cols = Est:Obs, values_to = "Richness",
                         names_to = "Source")
